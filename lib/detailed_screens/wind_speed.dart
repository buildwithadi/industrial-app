...
class WindSpeedDetailScreen extends StatefulWidget {
  final String deviceId;
  final String sessionCookie;

  const WindSpeedDetailScreen({
    super.key,
  ...
  @override
  State<WindSpeedDetailScreen> createState() => _WindSpeedDetailScreenState();
}

class _WindSpeedDetailScreenState extends State<WindSpeedDetailScreen> {
  // --- STATE ---
      ...
      if (response.statusCode == 200) {
        ...
        if (rawData.isNotEmpty) {
          ...
          for (var item in rawData) {
            // Parse 'wind_speed'
            double val = double.tryParse(item['wind_speed'].toString()) ?? 0.0;
  ...
  // --- AI Insights Logic for Wind ---
  String _getAIInsight() {
    if (_graphData.isEmpty) return "Gathering data to generate insights...";

    if (maxVal > 50) {
      return "🌪️ High Wind Alert. Gusts detected above 50 km/h. Avoid spraying pesticides. Secure loose structures.";
    } else if (avgVal > 20) {
      return "🌬️ Breezy Conditions. Good air circulation prevents fungal diseases, but may dry out soil faster.";
    } else if (avgVal < 3) {
      return "🍃 Calm Air. Low air movement detected. Check for poor ventilation in enclosed areas.";
    } else {
      return "✅ Gentle Breeze. Optimal wind conditions for pollination and general plant health.";
    }
  }
