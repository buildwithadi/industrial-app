...
class LightIntensityDetailScreen extends StatefulWidget {
  final String deviceId;
  final String sessionCookie;

  const LightIntensityDetailScreen({
    super.key,
  ...
  @override
  State<LightIntensityDetailScreen> createState() =>
      _LightIntensityDetailScreenState();
}

class _LightIntensityDetailScreenState
    extends State<LightIntensityDetailScreen> {
  // --- STATE ---
      ...
      if (response.statusCode == 200) {
        ...
        if (rawData.isNotEmpty) {
          ...
          for (var item in rawData) {
            // Parse 'light_intensity'
            double val =
                double.tryParse(item['light_intensity'].toString()) ?? 0.0;
  ...
  // --- AI Insights Logic for Light ---
  String _getAIInsight() {
    if (_graphData.isEmpty) return "Gathering data to generate insights...";

    if (avgVal > 50000) {
      return "☀️ Intense Light Detected. Direct sunlight exposure is high. Monitor for sunscald on sensitive crops.";
    } else if (avgVal < 1000) {
      return "🌑 Low Light Conditions. Photosynthesis may be limited. Check for obstructions or shade coverage.";
    } else if (maxVal > 80000) {
      return "⚠️ Peak Intensity Alert. Extremely high brightness recorded. Ensure sensors are calibrated.";
    } else if (avgVal > 10000 && avgVal < 30000) {
      return "✅ Optimal Light. Good balance of sunlight for vigorous growth.";
    } else {
      return "🌥️ Moderate Light. Sufficient for shade-tolerant plants but may need monitoring for fruit-bearing crops.";
    }
  }
...
class _DetailedChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (yRange == 0) {
      yRange = 100; // Larger default range for light
      minVal -= 50;
      maxVal += 50;
    } else {
      minVal -= yRange * 0.1;
    ...
    for (int i = 0; i <= 3; i++) {
      ...
      // Compact formatting for large light numbers (e.g. 1.2k)
      String labelText = labelVal >= 1000
