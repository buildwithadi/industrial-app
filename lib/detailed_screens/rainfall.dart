...
class _RainfallDetailScreenState extends State<RainfallDetailScreen> {
      ...
      if (response.statusCode == 200) {
        ...
        if (rawData.isNotEmpty) {
          ...
          for (var item in rawData) {
            // Parse 'rainfall'
            double val = double.tryParse(item['rainfall'].toString()) ?? 0.0;
  ...
  // --- AI Insights Logic for Rainfall ---
  String _getAIInsight() {
    if (_graphData.isEmpty) return "Gathering data to generate insights...";

    if (maxVal > 20) {
      return "⛈️ Heavy Rainfall Detected. Risk of waterlogging or erosion. Ensure drainage is clear.";
    } else if (avgVal == 0 && maxVal == 0) {
      return "☀️ Dry Spell. No rainfall detected in this period. Monitor soil moisture levels.";
    } else if (avgVal > 0 && avgVal < 2) {
      return "☁️ Light Drizzle. Consistent but low precipitation. Good for maintaining surface moisture.";
    } else {
      return "✅ Moderate Rainfall. Precipitation levels are normal for this season.";
    }
  }

  String _getAverageLabel() {
    if (_selectedRange == '24h') return "Avg Rate (Daily)";
    if (_selectedRange == '7d') return "Avg Rate (Weekly)";
    return "Avg Rate (Monthly)";
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Rainfall Details"),
        elevation: 0,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i <= 3; i++) {
      ...
      textPainter.text = TextSpan(
        text: labelVal.toStringAsFixed(1), // Changed to 1 decimal for rain
