...
class _PressureDetailScreenState extends State<PressureDetailScreen> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (rawData.isNotEmpty) {
          ...
          double sum = 0;
          double localMax = -999;
          double localMin = 9999; // Pressure can be around 1000, so high init
          String localMaxTime = "--";
          String localMinTime = "--";

          for (var item in rawData) {
            // Parse 'pressure'
            double val = double.tryParse(item['pressure'].toString()) ?? 0.0;

            // Skip invalid 0 readings for pressure usually
            if (val == 0) continue;
  ...
  // --- AI Insights Logic for Pressure ---
  String _getAIInsight() {
    ...
    if (avgVal < 1000) {
      return "☁️ Low Pressure System. Indicates unstable weather, possible storms or precipitation approaching.";
    } else if (avgVal > 1025) {
      return "☀️ High Pressure System. Indicates stable, clear, and dry weather conditions.";
    } else if ((maxVal - minVal) > 15) {
      return "⚠️ Rapid Pressure Change. Expect significant shift in weather conditions or wind speeds.";
    } else {
      return "✅ Stable Atmosphere. Pressure levels are normal (approx. 1013 hPa).";
    }
  }
...
class _DetailedChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Pressure usually doesn't swing wildly like rain, but needs range
    if (yRange < 2) {
      yRange = 10;
      minVal -= 5;
      maxVal += 5;
